...
//=============================================== *//
//ローカルサイトのURLに合わせて変更
const projectUrl = "http://salon-charm.local/";
...
function copyImage() {
  return gulp
    ...
    .pipe(browserSyncInstance.stream())
    .on("end", async () => {
      await deleteAsync(["./img/**/*.{png,jpg,jpeg}"]); // SVGは削除しない
    });
}
//* ===============================================
...
function browserInit(done) {
  browserSyncInstance.init({
    proxy: projectUrl, //必要に応じてprojectUrl変数を変更
    notify: false,
  });
  done();
}
